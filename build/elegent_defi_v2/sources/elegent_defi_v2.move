...
module elegent::elegent_defi_v2 {
    use std::signer;
    use std::vector;
    ...
    public entry fun refinance_loan(
        ...
        // Extract values from old loan before mutations
        let old_token_type = old_loan.token_type;
        let old_amount = old_loan.amount;
        let old_collateral_amount = old_loan.collateral_amount;
        let old_interest_rate = old_loan.dynamic_interest_rate;

        let trust_score = borrow_global<TrustScoreNFT>(borrower_addr);
        let new_interest_rate = calculate_dynamic_interest_rate(trust_score.score);
        
        // Only allow refinancing if new rate is better
        assert!(new_interest_rate < old_interest_rate, error::invalid_argument(E_INVALID_AMOUNT));

        // Create new loan with better terms
        let new_loan_id = platform_state.total_loans + 1;
        let new_interest_amount = (old_amount * new_interest_rate) / 10000;
        ...
        let new_loan = Loan {
            ...
            last_extended: 0,
            extension_count: 0,
            collateral_amount: old_collateral_amount,
        };
    ...
    public entry fun liquidate_loan(
        platform_admin: &signer,
        liquidator: address,
        loan_id: u64
    ) acquires PlatformState, TrustScoreNFT, EventHandles {
        let admin_addr = signer::address_of(platform_admin);
        ...
        coin::transfer<AptosCoin>(platform_admin, liquidator, liquidation_reward);
    ...
    public entry fun stake_apt(
        ...
        // Accrue rewards before updating stake (inlined)
        let time_diff = current_time - staking_pool.last_reward_time;
        if (time_diff > 0 && table::contains(&staking_pool.user_stakes, user_addr)) {
            let user_stake = *table::borrow(&staking_pool.user_stakes, user_addr);
            if (user_stake > 0) {
                let rewards = (user_stake * STAKING_REWARD_BPS * time_diff) / (10000 * 365 * 24 * 3600);
                let user_rewards = table::borrow_mut(&mut staking_pool.user_rewards, user_addr);
        ...
        *stake_time = current_time;
        ...
        event::emit_event(&mut event_handles.stake_events, StakeEvent {
            ...
            lock_until: current_time + staking_pool.lock_duration,
        });
    }
    ...
    public entry fun unstake_apt(
        ...
        let user_stake_val = *table::borrow(&staking_pool.user_stakes, user_addr);
        assert!(user_stake_val >= amount, error::invalid_argument(E_INSUFFICIENT_FUNDS));

        // Accrue rewards before unstaking (inlined)
        let current_time = timestamp::now_seconds();
        let time_diff = current_time - staking_pool.last_reward_time;
        if (time_diff > 0 && user_stake_val > 0) {
            let rewards = (user_stake_val * STAKING_REWARD_BPS * time_diff) / (10000 * 365 * 24 * 3600);
            let user_rewards = table::borrow_mut(&mut staking_pool.user_rewards, user_addr);
            *user_rewards = *user_rewards + rewards;
        };
        staking_pool.last_reward_time = current_time;
        ...
        *pending = *pending + amount;
        ...
        event::emit_event(&mut event_handles.stake_events, StakeEvent {
            ...
            amount,
            total_staked: *user_stake,
            lock_until: 0, // No lock for unstaking
        });
    }
    ...
    fun calculate_wallet_age(user_addr: address): u64 {
        // Simple calculation: return 30 days for existing wallets (in seconds)
        // In a real implementation, you would check the account creation time
